...
- `docs/profiles/*.json` - Generated per-SDK headline profiles and index, published with the page by the daily build
...
- curated/protocol-delivery.json: schema v1 `upgrades` entries have name, caps (name/url), mainnet_activation_date, activation_url, verified, notes, and releases keyed by SDK (tag/published_at/url/note). Only a valid ISO verified date plus valid activation date/evidence/CAP URLs opens the publication gate. Missing SDK release data renders n/a. Protocol 27 and 28 currently carry maintainer verification dates. Qualification is the first stable release announcing the usable protocol API, not preliminary XDR adoption; GitHub publication is the shipping proxy. Lag uses UTC calendar days against mainnet activation, without judgment colors.
- dashboard exports: `render_dashboard()` captures raw headline values at their rendering gates into `Signals`; snapshots and profiles only serialize those records. Never add a separate metric calculator. Definition identifier `dashboard-v1-response-v3` is distinct from export `schema_version: 1` and collector schema versions. See `dashboard/exports.md` for the complete shape and explicit differences from issue 80.
- profiles: each build uses the same loaded data for HTML and JSON; `docs/profiles/index.json` lists the active SDK files (four by default), removing obsolete disabled-SDK profiles. The daily workflow commits page and profiles together. Each signal carries value, unit, sample_size, window, coverage, source observation time, freshness, evidence links, and source files. A/N remain separate counts; missing page values export null/incomplete, genuine empty medians null/not_applicable. Percentiles, pool sizes, uncollected activity signals, and seven-day response alternatives are null with reasons. There is no comparison pool or combined score.
- export provenance: methodology records actual generation time, calculation `as_of`, full source commit, dirty-tree flag, live definitions URL and committed HTML definitions line. Source timestamps are never replaced with generation time. Coverage and 48-hour freshness remain independent; curated verification is historical evidence, not a daily freshness deadline. Local deterministic checks can reuse an earlier `NOW`/`TODAY` while recording actual generation time separately.
- snapshots: manual `workflow_dispatch` only; quarter must match `^20[0-9]{2}-Q[1-4]$`. The label is not a quarter filter. Copy all SDK and curated JSON byte-identically to `inputs/`, hash the bytes, compute from those frozen copies, and emit summary JSON, equivalent flat CSV, and two-paragraph immutable README. Refuse any existing quarter path without changing it; clean only a failed new capture. Remote-quarter checks precede each rebase/push attempt. No force input. Local checks must use a scratch output root, never a real repository snapshot or workflow commit/push steps.
